...
total_runs = 51;
test = 'test0';
test_folder = ['logs/psacmaes/' test];
mkdir(test_folder);
for dim = dimensions
    results_filename = sprintf([test_folder '/results-dim%d.csv'], dim);
    results_file = fopen(results_filename, 'w');
    fprintf(results_file, 'Index;Global Minimal;Best Fitness run; Worst Fitness run; Mean Fitness run; Median Fitness run; STD Fitness run; SucessRate\n');
    fclose(results_file);
    for function_to_eval = f_to_eval
        %% Create Logs
        empty_log.fevals = [];
        empty_log.fitness = [];
        empty_log.bestFitness = [];
        log_array = repmat(empty_log, total_runs,1);
        %% Problem Settings
        CostFunction=@(x)cec14_func(x',function_to_eval);   % Cost Function
        %CostFunction=@Ackley;
        
        VarSize=[1 dim];       % Decision Variables Matrix Size

        VarMin=-100;             % Lower Bound of Decision Variables
        VarMax= 100;             % Upper Bound of Decision Variable      
        ...
        for run = 1:total_runs
            ...
            lambda_default=(4+round(3*log(dim)))*10;
            ...
            w=w/sum(w);
            w=[w,zeros(1,lambda_default-mu)];
            
            disp(['Started Run ' num2str(run) ' of problem ' num2str(function_to_eval) ' in dim ' num2str(dim) ]);
            ps=cell(MaxIt,1);
            pc=cell(MaxIt,1);
            ...
            for g=1:MaxIt
                ...
                % Display Results
                %if mod(g, 10) == 0
                %    disp(['FEVALS ' num2str(nb_evals) ': Best Cost = ' num2str(BestCost(g)) ' Pop = ' num2str(lambda)]);
                %end
                   
                
                % Save Results in log if bestCost improved
                if g > 1
                    if abs(BestCost(g) - function_to_eval*100) < abs(BestCost(g-1) - function_to_eval*100)
                        log_array(run).fevals = [log_array(run).fevals nb_evals];
                        log_array(run).fitness = [log_array(run).fitness BestCost(g) - function_to_eval*100];
                        log_array(run).bestFitness = BestCost(g) - function_to_eval*100;
                        to_be_written = ['FEVALS ' num2str(nb_evals) ': Best Cost = ' num2str(BestCost(g) - function_to_eval*100) ...
                            ' Pop = ' num2str(lambda) '\n'];
                        fprintf(repmat('\b', 1, old_size_written(2)-1)); 
                        fprintf(to_be_written);
                        old_size_written = size(to_be_written);
                    end
                else
                    log_array(run).fevals = [log_array(run).fevals nb_evals];
                    log_array(run).fitness = [log_array(run).fitness BestCost(g) - function_to_eval*100];
                ...
                FIM_sqrt = sqrtm(ecmnfish(reshape([pop(1:lambda).Position], lambda, dim), C{g+1}));

                % Delta theta
                d_Sigma = sigma{g+1}^2 * C{g+1} - sigma{g}^2 * C{g};
                trildSigma = tril(d_Sigma);
                vechdSigma = trildSigma(trildSigma~=0);
                d_m = M(g+1).Position - M(g).Position;
                d_theta = [d_m, vechdSigma']';
                ...
                lambda_r = lambda_r * exp(beta_*(gamma_theta{g+1} + norm(sigma{g+1})^2/alpha_));
                ...
                %disp(['sigma old: ' num2str(sigma{g+1})])
                sigma{g+1} = sigma{g+1} * (scaling_factor_new/scaling_factor_old);
                %disp(['new sigma: ' num2str(sigma{g+1})])

            end
        end
        
        %% Save logs to file
        % Update CSV for Summaries
        results_file = fopen(results_filename, 'a');
        write_results_file = sprintf('%d;%d;%0.8f;%0.8f;%0.8f;%0.8f;%0.8f;%0.8f\n', ...
                                        function_to_eval, ...
                                        function_to_eval * 100, ...
                                        min([log_array.bestFitness]), ...
                                        max([log_array.bestFitness]), ...
                                        mean([log_array.bestFitness]), ...
                                        median([log_array.bestFitness]), ...
                                        std([log_array.bestFitness]), ...
                                        sum([log_array.bestFitness] < 1e-8)/total_runs);
        fprintf(results_file, write_results_file);
        fclose(results_file);
        % Create CSV File for FEVALS
        fevals_filename = sprintf([test_folder '/fevals-dim%d-prob%d.csv'], dim, function_to_eval);
        AG_nb_FC = [0.0 0.0001*MaxEvals 0.01*MaxEvals];
        for x = linspace(0.1, 1.0, 10)
            AG_nb_FC = [AG_nb_FC x*MaxEvals];
        end
        
        fevals_file = fopen(fevals_filename, 'w');
        fprintf(fevals_file, sprintf('AG_#FC_%d;', dim));
        fclose(fevals_file);
        dlmwrite(fevals_filename, AG_nb_FC, 'delimiter',';', '-append');
        
        fevals_file = fopen(fevals_filename, 'a');
        fclose(fevals_file);
        for run = 1:total_runs
            error_evolution_index = 1;
            error_evolution  = zeros(size(AG_nb_FC));
            for gen = 1:max(size(log_array(run).fevals))
                error_evolution(error_evolution_index) = log_array(run).fitness(gen);
                if log_array(run).fevals(gen) > AG_nb_FC(error_evolution_index)
                    error_evolution_index = error_evolution_index + 1;
                end
            end
            fevals_file = fopen(fevals_filename, 'a');
            fprintf(fevals_file, sprintf('Run%d;', run));
            fclose(fevals_file);
            dlmwrite(fevals_filename, error_evolution, 'delimiter',';', '-append', 'precision', 10);
            fevals_file = fopen(fevals_filename, 'a');
            fclose(fevals_file);
        end
